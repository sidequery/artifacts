...
This is a self-contained opt-in example, not part of the default Canvas installation. It has three pieces:

- `worker.ts` and `collector.ts`: one SQLite-backed Durable Object, shared polling and persisted last-known data; bundled with Canvas locally, or deployed separately on Cloudflare.
- `plugin.ts`: the `github-runners` deployment plugin, exposing `getStatus` to authenticated canvases and MCP clients.
- `runner-status.canvas.tsx`: the single-file React canvas. It reads the same plugin snapshot as an agent.

The collector holds the GitHub token. The Canvas app Worker holds only the collector URL and a separate collector access token. Browser code receives neither token. The plugin uses an explicit `(subject, authority)` allowlist; its checked-in configuration denies everyone until configured. Canvas backends cannot call plugin functions, so the collector is an application-owned Durable Object rather than an artifact backend.

## Run the self-contained example on celld

From the Canvas repository root:
...
Open the URL printed by the launcher (default `http://127.0.0.1:4786/runner-status`). It downloads and verifies the pinned celld executable if necessary, builds the example, starts **one native celld process containing both Canvas and its collector**, and creates the private canvas. No existing microapp, specific computer, Tailscale service, or Cloudflare deployment is required. GitHub is the only external data service. Bun and the repository dependencies are needed for building; GitHub CLI is optional when supplying `GITHUB_TOKEN`. Managed celld supports macOS arm64 and glibc Linux arm64/x64.
...
The local plugin permits only `{ subject: "local", authority: "local" }` and calls the bundled collector over authenticated loopback HTTP. The entire runtime binds to `127.0.0.1`. The singleton collector starts on the first authorized status request and polls every 45 seconds even when the browser closes, until celld stops. Tabs share its persisted snapshot. Connect an MCP client to `http://127.0.0.1:4786/mcp` to edit the canvas or invoke the same read-only plugin.

The following sections describe the optional Cloudflare deployment of the same collector and canvas.

## Deploy the collector

Run from the Canvas repository root after `bun install --frozen-lockfile`. Edit `examples/runner-status/wrangler.jsonc` with your organization, repository list and optional runner-name prefix. Set a unique Worker name if necessary.
...
Record the resulting `https://<collector-host>/api/status` URL. This endpoint requires `Authorization: Bearer <collector access token>` and accepts only GET. It serves data to the Canvas plugin, not directly to browsers.

## Install the Canvas plugin

Edit `examples/runner-status/canvas.plugins.ts`, replacing `allowedUsers: []` with the exact Canvas identities allowed to inspect the pool:

```ts
allowedUsers: [
  { subject: "YOUR_CANVAS_USER_ID", authority: "better-auth" },
]
```

For Better Auth, the user ID comes from `/api/session` while signed in; `authority` is `better-auth`. For Cloudflare Access, use the verified identity's JWT `sub` and issuer (`https://<team>.cloudflareaccess.com`). Do not substitute an email address for the subject. Everyone on this allowlist sees the same configured pool, independently of which canvas or library invokes the plugin.
...
```sh
# These target the main Canvas app Worker using the root Wrangler configuration.
bun x wrangler secret put RUNNER_STATUS_URL
bun x wrangler secret put RUNNER_STATUS_TOKEN

CANVAS_PLUGINS_CONFIG=examples/runner-status/canvas.plugins.ts bun run build:cloudflare-compiler
CANVAS_PLUGINS_CONFIG=examples/runner-status/canvas.plugins.ts bun run build:cloudflare
```

Set `RUNNER_STATUS_URL` to the collector's exact HTTPS `/api/status` URL and `RUNNER_STATUS_TOKEN` to the collector access token. Then deploy the Canvas compiler and app using your existing deployment configuration and the same `CANVAS_PLUGINS_CONFIG` value; see [Cloudflare deployment](../../docs/cloudflare.md). The GitHub credential belongs only on the collector.

## Create the canvas

Connect an MCP client to the authenticated **hosted** Canvas deployment. Call `canvas_guide`, then `canvas_write` with `name: "runner-status"` and the contents of `runner-status.canvas.tsx` as `contents`. This example has no artifact server source. In the gallery, select the canvas, set its URL slug to `runner-status`, keep access **Private**, and save the link.
...
Pass that object to `canvas_plugin_call`. The result includes runner/job data, per-repository freshness, runner freshness, refresh time and errors. There are no write, cancel or rerun operations.
...
bun x wrangler deploy --dry-run --config examples/runner-status/wrangler.jsonc --outdir /tmp/canvas-runner-status-build
```

The unit tests cover pagination, global concurrency, run deduplication, label matching, authorization, bounds and stale-data recovery. The workerd test verifies shared requests, cache persistence and alarm-driven polling after restart (about 45 seconds). Browser tests compile the real canvas and use synthetic snapshots to exercise navigation, filtering, steps, empty/error states and stale-data handling. These checks run through the normal repository test commands as well.
