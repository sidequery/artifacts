...
Canvas is one package with two execution paths. `canvas web`, filesystem commands,
and stdio MCP use Bun with local canvas files and a history database. The hosted
application uses Workers and Durable Objects; it runs on Cloudflare or locally
through celld (`canvas server`). Running the Worker application locally does not
...
| `src/service.ts`, `canvasFile.ts`, `history.ts`, `compile.ts`, `typecheck.ts` | Bun filesystem canvas operations |
...
| `src/mcp/` | Shared MCP tool schemas (`tools.ts`), canvas guide (`guide.ts`), and app contract (`app-contract.ts`); Bun tool dispatch (`local-tools.ts`), stdio framing/transport (`protocol.ts`, `stdio.ts`), and app HTML/local preview assembly (`app.ts`) |
| `src/sdk/` | Public canvas authoring API, components, hooks, and native server helpers |
| `src/runtime/` | Browser entrypoints and bridges for Herdr, MCP Apps, and hosted canvases |
...
schemas for hosted capabilities. `service.ts` coordinates canvas operations,
canvas activation, artifact links, and the combined gallery. `script-service.ts`
...
`src/cli.ts`. Open accepts an explicit canvas name or falls back to
`HERDR_CANVAS_NAME`; `--version` selects archived source without that fallback.
`src/herdr.ts` is the Herdr client, `src/open.ts` coordinates canvas opening, and
