...
const url = new URL(process.env.CANVAS_MCP_URL ?? "http://127.0.0.1:4785/mcp");
...
try {
  ...
  const result = await client.callTool({ name: "canvas_write", arguments: {
    name: example, contents: await Bun.file(new URL(`../examples/${example}.canvas.tsx`, import.meta.url)).text(),
    ...(example === "counter" ? { server: await Bun.file(new URL("../examples/counter.canvas.server.ts", import.meta.url)).text() } : {}),
  } });
