...
import type { ArtifactFile, ArtifactFileRequest } from "../src/sdk/files";

export const MAX_FILE_BYTES = 25 * 1024 * 1024;
const GRANT_LIFETIME_MS = 5 * 60 * 1000;
const ID = /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/;
export const TRANSFER_PATH = /^\/api\/(?:artifact|canvas)\/files\/transfer\/([0-9a-f]{64})\/([0-9a-f-]{36})$/;
type Grant = { method: "PUT" | "GET"; file: ArtifactFile; expires: number };
type FileReply = { files: ArtifactFile[]; cursor?: string } | { file: ArtifactFile; path: string; expires: string } | { deleted: true };
...
export function validateFileRequest(value: unknown): asserts value is ArtifactFileRequest {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new ArtifactFileError("Invalid file request");
  ...
  if (typeof input.operation !== "string" || !Object.hasOwn(allowed, input.operation)
    || Object.keys(input).some(key => !allowed[input.operation as string]!.includes(key))) throw new ArtifactFileError("Invalid file request");
  if (input.operation === "list") {
    if (input.cursor !== undefined && (typeof input.cursor !== "string" || input.cursor.length > 2048)) throw new ArtifactFileError("Invalid file cursor");
  } else if (input.operation === "upload") {
    if (typeof input.name !== "string" || !input.name.trim() || input.name === "." || input.name === ".."
      || /[/\\\u0000-\u001f\u007f]/.test(input.name) || new TextEncoder().encode(input.name).byteLength > 255) throw new ArtifactFileError("File name must be 1–255 bytes without slashes or control characters");
    if (typeof input.size !== "number" || !Number.isSafeInteger(input.size) || input.size < 0) throw new ArtifactFileError("Invalid file size");
    if (input.size > MAX_FILE_BYTES) throw new ArtifactFileError("File exceeds 25 MiB", 413);
    if (typeof input.type !== "string" || input.type.length > 255 || /[^\x20-\x7e]/.test(input.type)) throw new ArtifactFileError("Invalid file content type");
  } else if (typeof input.id !== "string" || !ID.test(input.id)) throw new ArtifactFileError("Invalid file ID");
}

function metadata(object: R2Object): ArtifactFile {
...
/** Native R2 stays in the trusted host. One object owns one library/workspace/artifact,
 * including historical previews; caller-provided filenames never select storage keys. */
export class ArtifactFiles extends DurableObject<FilesEnv> {
  ...
  // Keep the deployed R2 prefix so existing uploads remain accessible.
  private prefix() { return `canvases/${this.ctx.id.toString()}/`; }
  private key(id: string) { return this.prefix() + id; }

  async request(input: ArtifactFileRequest): Promise<FileReply> {
    validateFileRequest(input);
    if (!this.env.FILES) throw new ArtifactFileError("Artifact file storage is not configured", 503);
    if (input.operation === "list") {
    ...
    if (this.ctx.storage.sql.exec<{ n: number }>("select count(*) as n from file_grants").one().n >= 128) throw new ArtifactFileError("Too many pending file transfers; retry after five minutes", 429);
  ...
  private async info(id: string): Promise<ArtifactFile> {
    if (this.ctx.storage.sql.exec("select id from file_uploads where id = ?", id).toArray().length) throw new ArtifactFileError("File not found", 404);
    const object = await this.env.FILES.head(this.key(id));
    if (!object) throw new ArtifactFileError("File not found", 404);
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (!match || match[1] !== this.ctx.id.toString() || !ID.test(match[2]!)) throw new ArtifactFileError("Transfer not found", 404);
      ...
      if (!row) throw new ArtifactFileError("File transfer expired or already used", 404);
      ...
      if (grant.method === "PUT") {
        ...
        if (length === null || !/^\d+$/.test(length) || Number(length) !== grant.file.size) throw new ArtifactFileError("Upload size does not match its transfer grant");
        ...
        try {
          ...
          if (state?.canceled !== 0 || object.size !== grant.file.size) {
            ...
            settled = true;
            throw new ArtifactFileError(state?.canceled !== 0 ? "Upload was canceled" : "Upload size does not match its transfer grant", 409);
          }
          settled = true;
      ...
      headers.set("content-type", "application/json");
      return new Response(JSON.stringify({ error: error instanceof ArtifactFileError ? error.message : "File transfer failed" }), { status: error instanceof ArtifactFileError ? error.status : 500, headers });
    }
  }
}
