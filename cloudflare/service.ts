...
export class CloudCanvasService {
  constructor(readonly library: DurableObjectStub<CanvasLibrary>, readonly workspace: string,
    readonly backends: DurableObjectNamespace<CanvasBackend>, readonly libraryKey: string) {}
  ...
  async callTool(name: string, args: Record<string, unknown>): Promise<CallToolResult> {
    switch (name) {
      ...
      case "canvas_write": return this.mutationResult(await this.library.writeDraft({ workspace: this.workspace, name: args.name as string, source: args.contents as string, server_source: args.server as string | null | undefined }));
      case "canvas_edit": return this.mutationResult(await this.library.editDraft({ workspace: this.workspace, name: args.name as string, part: args.part as "client" | "server" | undefined, edits: args.edits as CanvasEdit[], expected_hash: args.expected_hash as string | undefined }));
