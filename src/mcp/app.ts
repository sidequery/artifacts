...
import { artifactIdFromFile, assertRegularArtifact, ensureArtifactFileName } from "../artifactFile";
...
import type { ArtifactAppPayload } from "./app-contract";

let shell: Promise<string> | undefined;
export function artifactAppHtml(): Promise<string> {
...
async function buildShell(): Promise<string> {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Sidequery Artifacts</title>
<style>
...
@media(hover:hover){#display-mode:hover{background:color-mix(in srgb,var(--artifact-background,#181818) 92%,transparent)}}
...
#artifact-viewport{min-height:0;min-width:0;overflow:auto;overscroll-behavior:contain;scrollbar-gutter:stable;flex:1 1 auto}
#artifact-viewport:focus-visible{outline:2px solid currentColor;outline-offset:-2px}
...
html[data-display-mode="fullscreen"] #artifact-shell,html[data-display-mode="pip"] #artifact-shell{height:100vh;height:100dvh;max-height:none}
...
<body><div id="artifact-shell"><div id="artifact-toolbar" hidden><button id="display-mode" type="button" aria-label="Expand artifact" title="Expand artifact"><span id="display-mode-icon" aria-hidden="true"></span></button></div><div id="artifact-viewport" tabindex="0" role="region" aria-label="Artifact"><div id="status" role="status">Waiting for artifact…</div><div id="root"></div></div></div><script type="module">${js.replace(/<\/script/gi, "<\\/script")}</script></body></html>`;
}
...
export async function artifactAppResult(service: ArtifactService, selection: { name?: string; version_id?: string; event_id?: string }) {
  ...
  const compiled = await compileArtifact(path, source, undefined, project);
  if (!compiled.ok || !compiled.js) return { ok: false, path, check: formatArtifactCheck(compiled.diagnostics), diagnostics: compiled.diagnostics };
  if (runtimeIdentity() !== runtime) throw new Error("Artifact SDK changed during compilation; retry");
  ...
  const history = new ArtifactHistory(historyPath(service.env));
  try {
