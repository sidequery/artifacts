...
test("gallery lists working copies and archived/deleted versions with an explicit all-projects filter", async () => {
  const dir = tempDir();
  writeArtifact(dir, "working", VALID_ARTIFACT);
  const dbPath = join(dir, "history.sqlite");
  const history = new ArtifactHistory(dbPath);
  const archived = history.capture({ workspace: dir, name: "deleted", sourcePath: join(dir, "deleted.artifact.tsx"), source: VALID_ARTIFACT, runtime: "test" });
  const other = history.capture({ workspace: dir + "-other", name: "elsewhere", sourcePath: join(dir + "-other", "elsewhere.artifact.tsx"), source: VALID_ARTIFACT, runtime: "test" });
  const server = await createArtifactServer({ artifactsDir: dir, historyPath: dbPath, gallery: true });
  try {
    const page = await (await fetch(server.url)).text();
    expect(page).toContain("Artifacts library");
    ...
    expect(await downloaded.text()).toBe(VALID_ARTIFACT);
    expect(await (await fetch(`${server.url}/api/source?name=working`)).text()).toBe(VALID_ARTIFACT);
...
test("gallery previews carry a self-contained build and isolated state, including archived source from another project", async () => {
  const dir = tempDir();
  const path = writeArtifact(dir, "working", VALID_ARTIFACT);
  const statePath = path.replace(".tsx", ".data.json");
  writeFileSync(statePath, '{"count":7}');
  const dbPath = join(dir, "history.sqlite");
  const history = new ArtifactHistory(dbPath);
  const other = history.capture({ workspace: dir + "-other", name: "deleted", sourcePath: join(dir + "-other", "deleted.artifact.tsx"), source: VALID_ARTIFACT, runtime: "test" });
  ...
  try {
    ...
    const bridge = JSON.parse(html.match(/window\.__artifacts = (.*);<\/script>/)![1]);
...
test("ordinary managed Artifact servers do not expose the cross-project gallery API", async () => {
  const dir = tempDir();
