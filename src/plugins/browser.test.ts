...
test("local checking uses prepared transitive declarations and rejects deep or uninstalled imports", async () => {
  const path = writeArtifact(tempDir(), "plugin", source);
  expect(typecheckArtifact(path, registry)).toEqual([]);
  ...
  expect(typecheckArtifact(writeArtifact(tempDir(), "invalid-sdk-type", invalidSdkType), registry).some(item => item.message.includes("invalid-tone"))).toBe(true);
  const invalid = writeArtifact(tempDir(), "plugin", source.replace('prefix: "Plugin"', "prefix: 123"));
  expect(typecheckArtifact(invalid, registry).some(item => item.message.includes("number"))).toBe(true);
  const hiddenImport = source.replace('from "@test/counter";', 'from "@test/counter"; import "jose";');
  expect((await compileArtifact(path, hiddenImport, registry)).ok).toBe(false);
  expect((await compileArtifact(path, 'import{base64url}from"jose";export default()=> <div/>', registry)).ok).toBe(false);
  for (const specifier of ["@test/counter/deep", "jose", "uninstalled"]) {
    const denied = source.replace("@test/counter", specifier);
    expect(typecheckArtifact(writeArtifact(tempDir(), "denied", denied), registry)[0]?.message).toContain("not allowed");
    expect((await compileArtifact(path, denied, registry)).ok).toBe(false);
  }
}, 30000);
...
test("installed package entries and explicit JavaScript declarations keep their public type contracts", async () => {
  ...
  const artifact = `import { base64url } from "@test/encoding";
import { labelLength } from "@test/untyped";
export default function Artifact() { return <div>{base64url.encode(String(labelLength({prefix: "ok"})))}</div>; }`;
  expect(typecheckArtifact(writeArtifact(tempDir(), "installed", artifact), prepared)).toEqual([]);
  expect(typecheckArtifact(writeArtifact(tempDir(), "invalid", artifact.replace('prefix: "ok"', 'prefix: false')), prepared)
    .some(item => item.message.includes("boolean"))).toBe(true);
  expect((await compileArtifact(writeArtifact(tempDir(), "installed", artifact), undefined, prepared)).ok).toBe(true);
}, 30000);
...
export default function Artifact() { return <div>{browser}</div>; }`;
  expect(typecheckArtifact(writeArtifact(directory, "conditional", source), prepared)).toEqual([]);
}, 30000);
