...
import { assertRegularCanvas, replaceCanvasSource } from "./canvasFile";
...
export function writeLocalProject(path: string, project: ArtifactProject, exclusive = false): void {
  ...
  if (existsSync(file)) assertRegularCanvas(file);
  replaceCanvasSource(file, contents, existsSync(file) ? readFileSync(file, "utf8") : undefined, "project update");
}
export function projectDiagnostics(path: string, source: string, project: ArtifactProject, publicImports: readonly string[]) {
  return Object.entries({ "canvas.canvas.tsx": source, ...project.files }).flatMap(([file, text]) => {
    ...
    return sandboxToDiagnostics(file === "canvas.canvas.tsx" ? path : file,
      scanCanvasSource(text, [...publicImports, ...allowed]).filter(item => file === "canvas.canvas.tsx" || item.kind !== "export"));
  });
}
...
export function materializeProject(source: string, input: ArtifactProject) {
  const project = normalizeProject(input), directory = realpathSync(mkdtempSync(join(tmpdir(), "canvas-project-")));
  try {
    for (const [file, contents] of Object.entries({ ...project.lock, ...project.files, "canvas.canvas.tsx": source })) {
      const path = join(directory, file);
      mkdirSync(dirname(path), { recursive: true }); writeFileSync(path, contents);
    }
    return { path: join(directory, "canvas.canvas.tsx"), directory, dispose: () => rmSync(directory, { recursive: true, force: true }) };
