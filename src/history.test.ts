...
test("history survives reconnect, scopes artifacts, and records distinct serves without duplicating unchanged source", () => {
  ...
  let history = new CanvasHistory(dbPath);
  const input = { workspace: dir, name: "one", sourcePath: join(dir, "one.canvas.tsx"), source: "raw TSX", runtime: "runtime-a" };
  ...
  history.close();
  history = new CanvasHistory(dbPath);
  try {
...
test("simultaneous processes share one revision for identical source", async () => {
  const dir = tempDir();
  const dbPath = join(dir, "history.sqlite");
  const initial = new CanvasHistory(dbPath);
  initial.close();
  const script = `import { CanvasHistory } from ${JSON.stringify(join(PLUGIN_ROOT, "src/history.ts"))};
    const history = new CanvasHistory(process.env.TEST_HISTORY_DB);
    for (let i = 0; i < 15; i++) history.capture({ workspace: process.env.TEST_WORKSPACE, name: 'shared', sourcePath: process.env.TEST_WORKSPACE + '/shared.canvas.tsx', source: 'same source', runtime: 'test' });
    history.close();`;
...
test("history path override does not depend on a pane's temporary plugin state directory", () => {
  const dir = tempDir();
  expect(historyPath({ HERDR_CANVAS_HISTORY_DB: join(dir, "archive.sqlite"), HERDR_PLUGIN_STATE_DIR: "/tmp/pane" })).toBe(join(dir, "archive.sqlite"));
